...
R"RAW_CUDA_CODE(
         ...
         float dt)
  {
    ...
    {
      //-- read --
      float px=points[id].px;
      float py=points[id].py;
      float vx=points[id].vx;
      float vy=points[id].vy;
      //-- compute --
      px+=(vx+xAccel*0.5f*dt)*dt;
      py+=(vy+yAccel*0.5f*dt)*dt;
      vx+=xAccel*dt;
...
R"RAW_CUDA_CODE(
         ...
         float yLimit)
  {
    ...
    {
      //-- read --
      const float px=points[id].px;
      const float py=points[id].py;
      const float vx=points[id].vx;
      const float vy=points[id].vy;
      //-- compute --
      const float r=(px+xLimit)/(2.0f*xLimit);
      const float g=(py+yLimit)/(2.0f*yLimit);
  ...
  // vecteur ``points''.
  //
  crs::CudaBuffer<Point> pointBuffer(device, pointCount);
  // ...
  ...
  //
  pointBuffer.fromHost(stream, data(points));
  // ...
  ...
  for(int i=0; i<stepCount; ++i)
  {
    ...
    //
    const void *gpuArgs[]={pointBuffer.programArg(),
                           &pointCount,
                           &xLimit, &yLimit,
                           &xAccel, &yAccel,
                           &dt};
  ...
  // correspondant sur le GPU.
  //
  pointBuffer.toHost(stream, data(points));
  // ...
