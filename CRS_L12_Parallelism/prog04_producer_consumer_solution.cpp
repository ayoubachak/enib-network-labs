             ...
             SharedData &sd)
{
  ...
  for(int iter=0; iter<100'000; ++iter)
  {
    ...
    {
      sd.cnd.wait(sd.mtx);  // block until condition is signaled
    }
    sd.fifo.enqueue(value); // then we can enqueue something
    sd.cnd.notify_all();    // wake-up any thread blocked on condition
    sd.mtx.unlock();
    // ...
     ...
     char **argv)
{
  ...
  {
    sd.tdP[i].th.join();
    const int amount=sd.tdP[i].amount;
    crs::out("producer[%]=%\n", i, amount);
    produced+=amount;
  }
  // ...
  ...
  {
    sd.tdC[i].th.join();
    const int amount=sd.tdC[i].amount;
    crs::out("consumer[%]=%\n", i, amount);
    consumed+=amount;
  }
  // ...
