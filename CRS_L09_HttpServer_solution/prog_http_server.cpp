  ...
  try
  {
    ...
    for(;;)
    {
      ...
      if((requestMethod=="POST")&&(requestUri=="/bin"))
      {
        ...
        // il faut alors mettre fin au dialogue.
        //
        if(crs::recvAll(dialogSocket, values, length)!=length)
        {
          throw std::runtime_error{"cannot receive binary content"};
        }
        // ...
        ...
        auto header=crs::txt("HTTP/1.1 200 OK\r\n"
                             ...
                             length);
        crs::sendAll(dialogSocket, header);
        crs::sendAll(dialogSocket, values, length);
        // ...
      ...
      if((requestMethod=="GET")&&(requestUri=="/")&&(upgrade=="websocket"))
      {
        ...
        for(;;)
        {
          ...
          if(crs::find(readSet, dialogSocket)!=-1)
          {
            ...
            auto [opcode, length]=
              crs::wsRecv(dialogSocket, buffer, sizeof(buffer));
            if(length==0)
            {
            ...
            if(opcode==crs::WS_TXT) //---- handle textual message ----
            {
              ...
              crs::out("ws_txt: <%>\n", msg);
              if(msg=="quit") // application wants to stop
              {
                //---- ask for closing ----
                crs::wsSendClose(dialogSocket);
            ...
            else if(opcode==crs::WS_BIN) //---- handle binary message ----
            {
              ...
              for(int i=0; i<valueCount; ++i)
              {
                msg+=crs::txt(" %", crs::ntoh_i32(values[i]));
              }
              crs::out("ws_bin:%\n", msg);
              // ...
            }
            else if(opcode==crs::WS_PING) //---- handle ping ----
            {
              ...
              //
              crs::wsSend(dialogSocket, buffer, length, crs::WS_PONG);
              // ...
            }
            else if(opcode==crs::WS_CLOSE) //---- handle client close ----
            {
              ...
              //
              crs::wsSend(dialogSocket, buffer, length, crs::WS_CLOSE);
              break; // stop websocket dialog
              // ...
            }
          }
          ...
          else
          {
            ...
            {
              elem=crs::hton_i32(elem);
            }
            crs::wsSend(dialogSocket, data(values), sizeof(values));
            // ...
          }
        }
...
#if !defined _WIN32 // avoid compilation problem (SOCKET!=int on Windows!)
          crs::dup2(dialogSocket, STDIN_FILENO);
          crs::dup2(dialogSocket, STDOUT_FILENO);
          crs::dup2(dialogSocket, STDERR_FILENO);
#endif
          crs::close(dialogSocket);
          crs::exec({path}); // never returns!
        }
        crs::waitpid(child);
        // ...
        if(connection=="close")
