...
txtDialogThread(SOCKET dialogSocket)
{
  try
  {
    ...
    for(PerfState state; state.next();)
    {
      ...
      {
        //---- each value is received in its textual form (11 chars) ----
        const std::string txtValue=crs::recvAll(dialogSocket, 11);
        if(empty(txtValue))
        {
          stop=true;
          break; // no more value to be received
        }
      ...
      //---- send this text to the client ----
      crs::sendAll(dialogSocket, txtSum);
    }
  }
...
binDialogThread(SOCKET dialogSocket)
{
  try
  {
    ...
    //---- send server endian information ----
    const int32_t sEndian=0x12345678;
    crs::sendAll(dialogSocket, &sEndian, sizeof(sEndian));

    //---- receive client endian information ----
    int32_t cEndian;
    if(crs::recvAll(dialogSocket, &cEndian, sizeof(cEndian))!=sizeof(cEndian))
    {
      throw std::runtime_error{"cannot receive client endian"};
    }
    if(sEndian==cEndian)
    {
    ...
    for(PerfState state; state.next();)
    {
      ...
      if(sEndian!=cEndian)
      {
        sum=crs::hton_i64(sum);
      }
      crs::sendAll(dialogSocket, &sum, sizeof(sum));
    }
  }
  catch(const std::exception &e)
  {
     ...
     char **argv)
{
  ...
  if(txtPortNumber!=0)
  {
    ...
    txtListenSocket=crs::socket(PF_INET, SOCK_STREAM, 0);
  ...
  if(binPortNumber!=0)
  {
    ...
    binListenSocket=crs::socket(PF_INET, SOCK_STREAM, 0);
    // ... avoiding timewait problems (optional)
  ...
  for(;;)
  {
    //---- wait for connections or UDP messages ----
    std::vector<SOCKET> readSet;
    if(txtPortNumber!=0)
    {
      readSet.emplace_back(txtListenSocket);
    }
    if(binPortNumber!=0)
    {
      readSet.emplace_back(binListenSocket);
    }
    crs::select(readSet);
