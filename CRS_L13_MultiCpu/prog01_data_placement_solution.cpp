           ...
           SharedData &sd)
{
  ...
  //
  auto workSpace=td.workSpace;
  for(int64_t w=workBegin; w<workEnd; ++w)
  {
    ++workSpace[w%workSpaceSize];
  }
     ...
     char **argv)
{
  ...
  {
    sd.td[i].th.join();
    for(const auto &w: sd.td[i].workSpace)
    {
      result+=w;
    }
  }
  ...
  crs::out("% iterations in % seconds (% it/s per cpu, % Joules)\n",
           result, duration,
           double(result)/duration/crs::len(sd.cpuInfo), energy);
